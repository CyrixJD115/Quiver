"""Headless application services for the CLI.

Every function here returns plain data (dicts/lists) and never touches a Rich
console: the CLI renders results. This module is the single source of truth.
Long-running operations accept an ``emit`` callable for ``(level, message)``
progress lines.
"""
...
from quiver.core import appimage, updater
from quiver.core.registry import AppEntry, Registry
from quiver.providers import known_providers, normalize_github_repo
from quiver.util import desktop, download, integrations, storage
from quiver.util.config import Config
from quiver.util.errors import NotFoundError, UsageError

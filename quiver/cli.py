...
and `timer` groups. Presentation is a thin layer over quiver.services.
"""
...
from quiver.core import maintenance, services, updater
from quiver.core.registry import AppEntry, Registry
from quiver.providers import known_providers
from quiver.util import download, integrations, storage
from quiver.util.config import Config, config_file_path, parse_config_value
from quiver.util.errors import AimError, CancelledError, NotFoundError, UsageError
from quiver.util.output import GLYPH_ERR, GLYPH_INFO, GLYPH_OK, GLYPH_WARN, UI, confirm
...
def root_callback(
    ...
    if ctx.invoked_subcommand is None:
        ...
        console.print(
            ...
            " [dim]Run [bold]quiver --help[/bold] for commands.[/dim]"
        )
